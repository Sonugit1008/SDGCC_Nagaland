...
class Sidebar extends Component {
    ...
    this.menuItems = menuItems
      ...
      .filter((menu) => {
        ...
        props.roles.forEach((role) => {
          if (menu.roles?.includes(role)) flag = true;
        });
  ...
  isRouteCanView(roles) {
    if (roles?.includes("all")) return true;

    let flag = false;

    this.props.roles.forEach((role) => {
      if (roles?.includes(role)) flag = true;
    });

    return flag;
  }

  isRouteCanViewSub(roles) {
    if (roles?.includes("all")) return true;
  ...
  process_roles = (menu_list, role) => {
    return menu_list.map((menu) => {
      // eslint-disable-line
      if (menu.subs) {
        // const { subs, ...menuitem } = menu;
        // const filtered_subs = subs.filter((sub) => sub.roles.includes(role));
        // if object has subs then first check object and then subs
      } else {
        // if object has no subs then just check objects
                ...
                this.menuItems.map((item) => {
                  return (
                    ...
                    >
                      {item.subs &&
                        item?.subs
                          .filter((sub) => this.isRouteCanView(sub.roles))
                          .map((sub, index) => {
                            return (
                              <NavItem
                                key={`${item.id}_${index}`}
                                  ...
                                  <Fragment>
                                    ...
                                    >
                                      <Nav className="third-level-menu">
                                        {sub.subs
                                          .filter((sub) =>
                                            this.isRouteCanViewSub(sub.roles)
                                          )
                                          .map((thirdSub, thirdIndex) => {
                                            return (
                                              <NavItem
