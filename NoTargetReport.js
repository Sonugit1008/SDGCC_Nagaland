...
class NoTargetIndicator extends Component {
  constructor(props) {
    super(props);
  ...
  viewDIFScore = () => {
    ...
    apiAuth
      .get(
        `/api/public/score/?category=No Target&type=${
  ...
  getRpeotPDf = () => {
    ...
    doc.save("NoTargetIndicatorReport.pdf");
  };
                  ...
                  <Select
                    options={
                      this.state.selectedType.value === 'DIF'
                        ...
                        ? this.state.years
                          ? this.state.years.filter(year => year >= 2018).sort((a, b) => b-a).map((year) => {
                              return {
                                label: `${year}-${String(Number(year) + 1).slice(2, 4)}`,
                                value: year,
                              };
                ...
                <>
                  <Colxx lg="1" className="pt-3">
                    <ExportToExcel
                      ...
                      fileName={`NoTargetIndicatorReport`}
                    />
                  </Colxx>
                  <Colxx lg="2" className="pt-3 ml-5" >
                    <Button
                      onClick={() => {
                        this.getRpeotPDf();
                      }}
...
export default connect(mapStateToProps, mapActionsToProps)(NoTargetIndicator);
